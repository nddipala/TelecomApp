/**
 * In-memory catalogs for the Device Service.
 *
 *  - eligibilityByAccount: upgrade eligibility keyed by account ID. In production
 *    this would be computed from contract status, device payment progress, and
 *    account standing. Here we precompute realistic outcomes.
 *  - tradeInValues: current promotional trade-in values keyed by a normalized
 *    device model name.
 *  - recommendedDevices: the upgrade catalog shown to eligible customers.
 */
...
// Phone numbers map to the same account-derived eligibility for convenience.
...
/** Resolve an account ID from either an account ID or phone number identifier. */
...
/** Normalize a model name so "iPhone 14", "iphone 14 " all match the catalog. */
